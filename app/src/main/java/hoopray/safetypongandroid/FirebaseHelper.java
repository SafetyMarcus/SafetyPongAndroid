...
public class FirebaseHelper
{
    ...
    {
        Firebase safetyPong = new Firebase(FirebaseConstants.FIREBASE_PATH + '/' + PLAYERS).child(SafetyPong.getInstance().currentLeagueKey).child(key);
        safetyPong.setValue(player);
    }

    public static void saveGame(String key, Game game)
    {
        Firebase safetyPong = new Firebase(FirebaseConstants.FIREBASE_PATH + '/' + GAMES).child(SafetyPong.getInstance().currentLeagueKey).child(key);
